...
class TrellisFirstStagePipeline:
    def __init__(self, output_path, work_dir, render_params, n_views=2,  steps=12):
    ...
    def set_cam_offset(self, offset=(-45, 30)):
        self.render_params['offset'] = offset

    def render_mesh(self, input_mesh_path):
        renderer = BlenderRenderer(resolution=1024)
        logging.info(f"Rendering mesh: {input_mesh_path} with params: {self.render_params}")
        ...
        images = renderer.render(input_mesh_path, save_dir=self.inputs_dir, n_views=self.n_views, mode='linear', lin_view_args=view_args)
    ...
    def run_pipeline(self, images, input_mesh_name):
        logging.info(f"Running the pipeline for {input_mesh_name}...")   

        coords = self.pipeline.run_multi_image_first_stage(
            images,
            seed=1,
            ...
            mode='multidiffusion'  # 'stochastic' or 'multidiffusion'
        )
        torch.cuda.empty_cache()

        outputs = self.pipeline.run_multi_image_second_stage(
            images, 
            coords,
            seed=1, 
            slat_sampler_params={
    ...
    def export_mesh(self, outputs, mesh_name):
        ...
        if glb:
            ...
            vertices, faces = postprocessing_utils.postprocess_mesh(vertices, faces, simplify_ratio=0.95)
    ...
    def run_for_mesh(self, input_mesh_path):
        ...
        images = self.render_mesh(input_mesh_path)
        gt_voxel_path = input_mesh_path.replace('.obj', '_voxels.ply')
        pred_voxel_path, outputs = self.run_pipeline(images, save_name)
...
if __name__ == "__main__":
    ...
    if 'work_dir' not in locals():
        ...
        print(f"Found {total_dirs} sub-directories in {dataset_dir}")

        with tqdm(total=total_dirs, desc="Processing folders") as pbar:
            for i in dataset_range:
                parent_dir = os.path.join(dataset_dir, f"Assembly_Dataset_{i}")
                render_params = {
                    "offset": (-45, 30),
                    "step": 30, # Viewing angle step for multi-views
                    "direction": 'left',
                    "set_fov": 30
                }
                pipeline = TrellisFirstStagePipeline(output_path, parent_dir, render_params, n_views=12, steps=12) 
        ...
        pipeline = TrellisFirstStagePipeline(output_path, work_dir, render_params, n_views=12 ,steps=12)
        pipeline.load_pipeline()
        pipeline.set_work_dir(work_dir)
        logging.info(f"Pipeline loaded successfully.")
